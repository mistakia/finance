...
export const parse_transactions = async ({ file_path, owner }) => {
  ...
  const institution = 'american-express'
  ...
  for (const record of records) {
    ...
    // Amex uses opposite convention: positive = purchase, negative = credit
    const amount = record.Amount ? -parseFloat(record.Amount.trim()) : null
