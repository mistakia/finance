...
const extract_account_id = (file_path) => {
  const filename = file_path.split('/').pop()
  const match = filename.match(/ally_(?:new_)?(.+?)_(\d{4})_/)
  if (match) {
    return match[2]
  }
  return 'default'
...
export const parse_transactions = async ({ file_path, owner }) => {
  ...
  const account_id = extract_account_id(file_path)
  const institution = 'ally-bank'
  const account_link = `/${owner}/${institution}/checking/${account_id}`
  ...
  for (const record of records) {
    if (!record || !record.Date || !record.Amount) continue

    const amount = parseFloat(record.Amount)
    if (isNaN(amount)) continue

    const date = parse_date(record.Date)
    ...
    transactions.push({
      ...
      transaction_time: record.Time || null,
